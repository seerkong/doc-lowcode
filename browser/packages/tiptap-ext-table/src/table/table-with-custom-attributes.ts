...
import { createColGroup } from './utilities/createColGroup.ts'
import { createTable } from './utilities/createTable.ts'
import { deleteTableWhenAllCellsSelected } from './utilities/deleteTableWhenAllCellsSelected.ts'

export interface TableWithCustomAttributesOptions {
  /**
...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    table: {
       ...
       */
      insertTable: (options?: { rows?: number; cols?: number; withHeaderRow?: boolean; HTMLAttributes?: Record<string, any> }) => ReturnType
...
/**
 * This extension allows you to create tables with custom attributes like x-id.
 * It extends the original Table extension to support custom attributes.
 * @see https://www.tiptap.dev/api/nodes/table
 */
export const TableWithCustomAttributes = Node.create<TableWithCustomAttributesOptions>({
  name: 'table',
  ...
  addAttributes() {
    return {
      'x-id': {
        default: null,
        parseHTML: element => element.getAttribute('x-id'),
        renderHTML: attributes => {
          if (!attributes['x-id']) {
            return {}
          }
          return { 'x-id': attributes['x-id'] }
        },
      },
    }
  },
    ...
    return [
      { 
        ...
        getAttrs: element => {
          ...
          if (element.hasAttribute('x-id')) {
            attrs['x-id'] = element.getAttribute('x-id')
          }
          return attrs
        }
      }
  ...
  addCommands() {
    return {
      insertTable:
        ({ rows = 3, cols = 3, withHeaderRow = true, HTMLAttributes = {} } = {}) =>
        ({ tr, dispatch, editor }) => {
          const node = createTable(editor.schema, rows, cols, withHeaderRow)
          
          // 设置自定义属性
          if (HTMLAttributes['x-id']) {
            node.attrs['x-id'] = HTMLAttributes['x-id']
          }
