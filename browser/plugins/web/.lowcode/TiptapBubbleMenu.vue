<template>
  <component
    v-if="BubbleMenuComponent"
    :is="BubbleMenuComponent"
    :editor
    :tippyOptions="{ maxWidth: 400 }"
    :updateDelay="100"
  >
...
import { computed, defineComponent, PropType, ref, getCurrentInstance, watchEffect, h, onMounted } from 'vue'
...
// 动态导入的BubbleMenu组件
const BubbleMenuComponent = ref(null)

// 动态导入BubbleMenu组件
onMounted(async () => {
  try {
    const module = await import('@tiptap/vue-3')
    BubbleMenuComponent.value = module.BubbleMenu
  } catch (error) {
    console.error('Failed to load BubbleMenu component:', error)
  }
})
